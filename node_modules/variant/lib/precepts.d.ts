 ...
 */
export declare type Variant<Type extends string, Fields extends {} = {}, Key extends string = 'type'> = Record<Key, Type> & Fields;
/**
 ...
 */
export declare type VariantCreator<T extends string, F extends (...args: any[]) => {} = () => {}, K extends string = 'type'> = ((...args: Parameters<F>) => PatchObjectOrPromise<ReturnType<F>, Record<K, T>>) & Outputs<K, T> & Stringable<T>;
/**
 * Given a VariantCreator, extract the output type. Unpack it
 * from a promise if it is inside one.
 */
export declare type CreatorOutput<VC extends VariantCreator<string, Func, string>> = ReturnType<VC> extends PromiseLike<infer R> ? R extends Record<VC['key'], string> ? R : never : ReturnType<VC>;
/**
 ...
 */
export declare type VariantModule<K extends string> = {
 ...
 */
export declare type TypeMap<T extends VariantModule<string>> = {
    [P in keyof T]: T[P]['type'];
};
/**
 * Reverse lookup - get the label from the literal type.
 */
export declare type GetTypeLabel<T extends VariantModule<string>, Key extends TypesOf<T>> = {
 ...
 */
export declare type TypeNameLookup<T extends VariantModule<string>> = {
    [P in TypesOf<T>]: GetTypeLabel<T, P>;
};
/**
 * Get the literal union for a variant's type property.
 */
export declare type TypesOf<T extends VariantModule<string>> = TypeMap<T>[keyof T];
/**
 * Get the literal union for a variant type's names, plus `undefined`.
 */
export declare type TypeNames<T extends VariantModule<string>> = TypesOf<T> | undefined;
/**
 * Simple internal helper to extract the variation types for each key of a `VariantModule`
 */
declare type VariantTypeSpread<T extends VariantModule<string>> = {
    [P in keyof T]: CreatorOutput<T[P]>;
};
/**
 * A union of variation types from any arbitrary `VariantModule`
 */
export declare type SumType<T extends VariantModule<string>> = Identity<VariantTypeSpread<T>[keyof T]>;
/**
 ...
 * ```
 */
export declare type VariantOf<T extends VariantModule<string>, TType = undefined> = TType extends undefined ? SumType<T> : TType extends TypesOf<T> ? Extract<SumType<T>, Record<T[keyof T]['key'], TType>> : SumType<T>;
/**
 ...
 */
export declare type Limited<T, U> = IsNever<Exclude<keyof T, U>> extends true ? T : VariantError<['Expected keys of handler', keyof T, 'to be limited to possible keys', U]>;
/**
 ...
 */
export declare type Splay<T> = {
    [P in keyof T]: Identity<Partial<T> & Record<P, T[P]>>;
}[keyof T];
export {};
//# sourceMappingURL=precepts.d.ts.map